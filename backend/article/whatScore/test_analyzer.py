...
def load_models():
    # 모델 파일 경로
    view_model_path = os.path.join(current_directory, 'backend/article/whatView/linear_regression_model.pkl')
    tendency_model_path = 'model_state_dict.pth'  # 수정된 부분
    ...
    if not os.path.exists(tendency_model_path):
        model_url = "https://drive.google.com/uc?id=1R6Vs8WRDGMVAQLdcK2gGU6AcI4FHy4-1"  # 파일 ID를 포함한 URL로 교체
        gdown.download(model_url, tendency_model_path, quiet=False)
    
    state_dict = torch.load(tendency_model_path)
    tendency_model = BertForClassification(bert_model).load_state_dict(state_dict)
    tendency_model.eval()
...
def analyze_url(url):
    ...
    vectorizer = TfidfVectorizer(stop_words='english', max_features=5000) # load vectorizer
    
    # 관점 분석
    view_title = remove_view_pattern(news_title)
    input = vectorizer.fit_transform(view_title)
    ...
    # 1개년 분석 파일 로드
    news_data = pd.read_csv("./1year_analysis.csv")
    ...
    plt.text(0.5, -0.11, f'{news_stance} 기사이며, {article_view}인 시각으로 작성되었습니다.',
            ha='center', va='top',
